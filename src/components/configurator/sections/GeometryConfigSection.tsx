...
export function GeometryConfigSection() {
  ...
  return (
    <SectionCard
      index={1}
      title="Géométrie"
      subtitle="Définissez l'emprise et la hauteur de l'immeuble"
      accent="#4A7FA8"
      metrics={
        <div className="flex gap-2 flex-wrap">
          <MetricPill label="Surface totale" value={surface.toFixed(0)} unit="m²" accent="#4A7FA8" />
          <MetricPill label="Volume" value={volume.toFixed(0)} unit="m³" accent="#4A7FA8" />
          <MetricPill label="Hauteur" value={(geometry.nFloors * geometry.floorHeight).toFixed(1)} unit="m" accent="#4A7FA8" />
        </div>
      }
    >
      ...
      <div>
        ...
        <div className="flex gap-2">
          {[1, 2, 3, 4, 5, 6, 8, 10].map(n => (
            <m.button
              key={n}
              onClick={() => updateGeometry({ nFloors: n })}
      ...
      <TechSlider label="Orientation façade principale" value={geometry.orientation} min={0} max={360} step={5} unit="°" decimals={0}
