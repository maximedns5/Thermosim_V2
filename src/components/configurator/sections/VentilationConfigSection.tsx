...
export function VentilationConfigSection() {
  ...
  return (
    <SectionCard index={5} title="Ventilation" subtitle="HRV/ERV system, airtightness and hygienic airflow" accent="#4A8A4A"
      metrics={
        <div className="flex gap-2 flex-wrap">
          <MetricPill label="n50" value={ventilation.n50} unit="ACH" accent="#4A8A4A" critical={ventilation.n50 > 7} />
          <MetricPill label="Heat recovery" value={recup} accent="#0B7A63" />
          <MetricPill label="Airflow" value={ventilation.q_v_hygienique} unit="m³/h" accent="#4A7FA8" />
        </div>
      }>
      <MaterialDropdown label="Ventilation system" value={ventilation.vmcId} options={VMC_OPTS}
        onChange={v => updateVentilation({ vmcId: v })}
        hint={selectedVmc ? `Heat recovery ${recup} · ${selectedVmc.conso_W_m3h} W/(m³/h)` : undefined} />
      <MaterialDropdown label="Airtightness level" value={selectedAirtight?.id ?? 'standard'} options={AIR_OPTS}
        onChange={v => { const a = AIRTIGHTNESS_LEVELS[v]; if (a) updateVentilation({ n50: a.n50 }); }} />
      <TechSlider label="n50 (blower door test)" value={ventilation.n50} min={0.3} max={20} step={0.1} unit="ACH" decimals={1}
        onChange={v => updateVentilation({ n50: v })} critical={ventilation.n50 > 7} />
      <TechSlider label="Hygienic airflow" value={ventilation.q_v_hygienique} min={0} max={1000} step={10} unit="m³/h" decimals={0}
