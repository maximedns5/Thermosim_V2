...
const ROOF_TYPES = [
  { value: 'flat_concrete',  label: 'Terrasse béton', icon: '▬', color: '#8A8680' },
  { value: 'green',          label: 'Toiture verte',  icon: '🌿', color: '#4A8A4A' },
  { value: 'inclined_tiles', label: 'Tuiles inclinées', icon: '⛺', color: '#8B4030' },
...
export function RoofConfigSection() {
  ...
  return (
    <SectionCard
      index={4}
      title="Toiture"
      subtitle="Type de toiture et isolation en tête de bâtiment"
      accent="#8B4030"
      metrics={
        <div className="flex gap-2 flex-wrap">
          <MetricPill label="U toiture" value={U_roof} unit="W/(m²·K)" accent="#8B4030" critical={U_roof > 0.5} />
          <MetricPill label="Épaisseur" value={(insulThick * 100).toFixed(0)} unit="cm" accent="#6B6B6B" />
        </div>
      }
    >
      {/* Sélecteur de type visuellement riche */}
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {ROOF_TYPES.map(rt => (
            <m.button
              key={rt.value}
              onClick={() => updateRoof({ type: rt.value })}
      ...
      <MaterialDropdown label="Isolant toiture" value={insulMat} options={INSUL_OPTS}
        onChange={v => store.setConfig({ ...config, roof: { ...roof, insulation: { material: v, thickness: insulThick } } })} />
      <TechSlider label="Épaisseur isolant" value={insulThick} min={0.04} max={0.40} step={0.01} unit="m" decimals={3}
