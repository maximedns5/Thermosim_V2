// SectionCard — glassmorphism card with scroll reveal
...
const ACCENTS = ['#4A7FA8','#0B7A63','#C1440E','#8B4030','#4A8A4A','#7060A8','#1A3550'];
  ...
  return (
    <section ref={ref} className="min-h-screen flex flex-col justify-center px-10 py-16">
      <m.div initial={{ opacity: 0, x: -24 }} animate={inView ? { opacity: 1, x: 0 } : { opacity: 0, x: -24 }}
        ...
        <span className="font-mono text-xs tracking-widest" style={{ color: `${color}99` }}>
      ...
      <m.h2 initial={{ opacity: 0, y: 28, filter: 'blur(8px)' }}
        ...
        style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 'clamp(2rem, 4vw, 3.2rem)', color: 'var(--color-ink)', letterSpacing: '-0.02em' }}>
        {title}
      </m.h2>
      ...
      <m.p initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 16 }}
        ...
        className="mb-8"
        style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: 'var(--color-ink-3)', letterSpacing: '0.02em' }}>
        {subtitle}
      </m.p>

      <m.div initial={{ opacity: 0, y: 32, scale: 0.97 }}
        animate={inView ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: 32, scale: 0.97 }}
        transition={{ duration: 0.6, delay: 0.22, ease: [0.16, 1, 0.3, 1] }}
        className="rounded-sm p-6 space-y-4"
        style={{
          background: 'rgba(242,239,232,0.7)', backdropFilter: 'blur(16px)',
      ...
      {metrics && (
        <m.div initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 12 }}
          transition={{ duration: 0.45, delay: 0.38 }} className="mt-4">
          {metrics}
        </m.div>
      )}
    </section>
